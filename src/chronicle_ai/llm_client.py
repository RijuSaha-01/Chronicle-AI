...
def generate_narrative(raw_text: str) -> str:
    """
    Generate a narrative paragraph from raw diary text.
    
    Uses Ollama Llama 3.2 to transform diary entries into engaging
    narrative prose. Falls back to the raw text if Ollama is unavailable.
    ...
    prompt = f"""You are a creative writer helping to transform personal diary entries into engaging narrative prose.
...
Narrative (2-4 sentences, cinematic style):"""
    ...
    return f"[Demo narrative – Ollama offline] {raw_text[:200]}{'...' if len(raw_text) > 200 else ''}"
